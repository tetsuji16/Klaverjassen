...
export default function TutorialPage({ progress, onProgress }: { progress: TutorialProgress; onProgress: (next: TutorialProgress) => void }) {
  ...
  if (lesson) return <div className="page narrow lesson-page">
    ...
    <div className="lesson-visual"><span>{lesson.id === 2 ? "J  ›  9  ›  A" : lesson.id === 8 ? "K  +  Q" : lesson.id === 12 ? "8 − 3 − 2" : "♣  ♦  ♥  ♠"}</span></div>
    <p className="lesson-summary">{lesson.summary}</p><p className="lesson-detail">{lesson.detail}</p>
    <div className="lesson-notes"><div><p className="eyebrow">KEY POINTS</p><ul>{lesson.keyPoints.map((point) => <li key={point}>{point}</li>)}</ul></div><div className="lesson-example"><p className="eyebrow">EXAMPLE</p><p>{lesson.example}</p></div></div>
    ...
    {progress.completed.includes(lesson.id) && lesson.id < lessons.length && <button className="button secondary wide" onClick={() => { setSelected(lesson.id + 1); setChoice(null); setChecked(false); window.scrollTo({ top: 0, behavior: "smooth" }); }}>次のレッスンへ →</button>}
    {lesson.id === lessons.length && progress.completed.includes(lesson.id) && <Link to="/cpu" className="button primary wide">卒業演習を始める →</Link>}
  </div>;
  return <div className="page tutorial-page"><div className="section-heading"><div><p className="eyebrow">LEARN THE GAME</p><h1>14のステップで、<br />遊びながら覚える。</h1><p className="intro">基本ルールから得点計算、実戦の考え方まで。好きなレッスンから始められます。</p></div><div className="progress-ring"><b>{progress.completed.length}</b><span>/ {lessons.length}</span><small>完了</small></div></div>
